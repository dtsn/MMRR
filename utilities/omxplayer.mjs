...
function Omx (source, output, loop, layout, initialVolume, showOsd) {
	...
	function spawnPlayer (src, out, loop, layout, initialVolume, showOsd) {

		let args = buildArgs(src, out, loop, layout, initialVolume, showOsd);
	...
	if (source) {
		player = spawnPlayer(source, output, loop, layout, initialVolume, showOsd);
	}

	// ----- Methods ----- //

	// Restarts omxplayer with a new source.
	omxplayer.newSource = (src, out, loop, layout, initialVolume, showOsd) => {

		if (open) {

			player.on('close', () => { player = spawnPlayer(src, out, loop, layout, initialVolume, showOsd); });
			player.removeListener('close', updateStatus);
			writeStdin('q');

		} else {

			player = spawnPlayer(src, out, loop, layout, initialVolume, showOsd);
